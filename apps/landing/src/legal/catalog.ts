 ...
 * Company fields are overridable from yonetim legal.company.
 */
...
export type LegalDocType = (typeof LEGAL_DOC_TYPES)[number]['type']

export const CONSENT_PACKS: Record<string, LegalDocType[]> = {
  purchase: [
    'service_agreement',
    'terms_of_use',
    'privacy_policy',
...
function sections(title: string, paras: string[]) {
  return [`## ${title}`, ...paras.map((p) => p)].join('\n\n')
}

/** Build markdown body for a document type (v1 drafts). */
export function buildDraftBody(type: LegalDocType, company = DEFAULT_LEGAL_COMPANY) {
...
export function resolveDocBySlug(slug: string) {
  const clean = String(slug || '')
    .replace(/^\//, '')
    .trim()
  return LEGAL_DOC_TYPES.find((d) => d.slug === clean || d.aliases.includes(clean as never)) || null
}

export function pathForType(type: LegalDocType) {
  const row = LEGAL_DOC_TYPES.find((d) => d.type === type)
  return row ? `/${row.slug}` : '/'
}
