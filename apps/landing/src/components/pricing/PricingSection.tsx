...
type KontorPackageId = (typeof kontorPackages)[number]['id']

function checkoutHref(period: BillingPeriod, kontorId?: KontorPackageId | null) {
  const params = new URLSearchParams({ plan: plan.id, period })
  if (kontorId) params.set('kontor', kontorId)
          ...
          <div className="grid grid-cols-1 items-stretch gap-6 lg:grid-cols-12 lg:gap-8">
            ...
            <article className="relative flex h-full min-h-0 lg:col-span-7">
              <div className="relative flex h-full w-full flex-col overflow-visible rounded-[32px] border-[3px] border-[#2563EB] bg-white p-7 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-9">
                ...
                <Link
                  to={checkoutHref(period, selectedKontorId)}
            ...
            {/* Sağ — kontör paketleri */}
            <aside className="flex h-full min-h-0 lg:col-span-5">
        ...
        return (
          ...
          >
            ...
            {option.id === 'year' ? (
              <span
                className={[
                  'ml-1.5 rounded-full px-1.5 py-0.5 text-[10px] font-extrabold tracking-wide uppercase',
