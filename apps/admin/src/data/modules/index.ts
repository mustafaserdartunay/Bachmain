...
function genRows(moduleId: string) {
  const templates: Record<string, () => Record<string, unknown>[]> = {
    ...
    subscriptions: () =>
      customers
        .filter((c: (typeof customers)[0]) => c.status !== 'churned')
        .map((c: (typeof customers)[0], i: number) => ({
          id: `sub${i}`,
          customer: c.company,
          plan: c.plan,
    ...
    dealers: () =>
      Array.from({ length: 8 }, (_, i) => ({
        ...
        city: ['Ankara', 'İstanbul', 'İzmir', 'Antalya', 'Trabzon', 'Konya', 'Gaziantep', 'Edirne'][
          i
        ],
        customers: [24, 18, 15, 12, 8, 10, 6, 5][i],
        commission: `%${[15, 12, 10, 12, 8, 10, 8, 8][i]}`,
        status: 'Aktif',
        contact: `Yetkili ${i + 1}`,
      })),
    accounts: () =>
      customers.map((c: (typeof customers)[0], i: number) => ({
        id: `acc${i}`,
        customer: c.company,
        debit: formatCurrency(c.balance > 0 ? c.balance : 0),
        credit: formatCurrency(c.balance < 0 ? Math.abs(c.balance) : 0),
        balance: formatCurrency(Math.abs(c.balance)),
        lastTransaction: '2026-07-08',
      })),
    payments: () => [
      {
        id: 'pay1',
        customer: 'Erlenbox Teknoloji',
      ...
      {
        id: 'pay2',
        customer: 'Atlas İnşaat',
        amount: formatCurrency(62000),
        method: 'EFT',
        date: '2026-07-05',
        status: 'Gecikmiş',
      },
      ...
      {
        id: 'pay5',
        customer: 'Titan Makine',
        amount: formatCurrency(19500),
        method: 'EFT',
        date: '2026-07-12',
        status: 'Bekleyen',
      },
      {
        id: 'pay6',
        customer: 'Meridian Yazılım',
        amount: formatCurrency(38000),
        method: 'Havale',
        date: '2026-06-25',
        status: 'Ödendi',
      },
    ...
    invoices: () =>
      Array.from({ length: 10 }, (_, i) => ({
        id: `inv${i}`,
        number: `FTR-2026-${String(150 - i).padStart(4, '0')}`,
    ...
    support: () =>
      supportTickets.map((t: (typeof supportTickets)[0]) => ({
        id: t.id,
        subject: t.subject,
        customer: t.customer,
        priority: statusLabels[t.priority],
    ...
    'live-support': () => [
      {
        id: 'ls1',
        customer: 'Nova Gıda San.',
        agent: 'Emre K.',
        waitTime: '2dk',
        topic: 'Kurulum yardımı',
        status: 'Aktif',
      },
      {
        id: 'ls2',
        customer: 'Vega Perakende',
        agent: 'Aylin D.',
        waitTime: '5dk',
        topic: 'Fatura sorunu',
        status: 'Bekliyor',
      ...
      {
        id: 'ls3',
        customer: 'Sirius Danışmanlık',
        agent: 'Can B.',
        waitTime: '0dk',
        topic: 'API entegrasyon',
        status: 'Aktif',
      },
    ],
    notifications: () =>
      Array.from({ length: 10 }, (_, i) => ({
        ...
        sent: `${10 - i} Tem 2026`,
        recipients: `${[248, 142, 248, 58, 12][i % 5]}`,
        status: ['Gönderildi', 'Taslak', 'Planlandı'][i % 3],
      })),
    ai: () => [
      {
        id: 'ai1',
        feature: 'Fatura Analizi',
        queries: '12.4K',
        tokens: '8.2M',
        cost: formatCurrency(4100),
        status: 'Aktif',
      },
      {
        id: 'ai2',
        feature: 'Stok Tahmini',
        queries: '8.7K',
        tokens: '5.1M',
        cost: formatCurrency(2550),
        status: 'Aktif',
      },
      {
        id: 'ai3',
        feature: 'Müşteri Segmentasyonu',
        queries: '3.2K',
        tokens: '2.8M',
        cost: formatCurrency(1400),
        status: 'Aktif',
      },
      {
        id: 'ai4',
        feature: 'Doğal Dil Sorgu',
        queries: '15.1K',
        tokens: '12.4M',
        cost: formatCurrency(6200),
        status: 'Aktif',
      },
      {
        id: 'ai5',
    ...
    analytics: () => [
      ...
      {
        id: 'an3',
        metric: 'En Çok Kullanılan Modül',
        value: 'Stok',
        change: '—',
        period: 'Son 30 gün',
      },
    ...
    security: () => [
      {
        ...
        source: '185.27.xxx.89',
        severity: 'Orta',
        date: '2026-07-09 16:45',
        status: 'İzleniyor',
      },
      {
        id: 'sec2',
        event: 'API key rotasyonu',
        source: 'Sistem',
        severity: 'Düşük',
      ...
      {
        id: 'sec4',
        event: '2FA zorunluluğu ihlali',
        source: 'Zenith Medikal',
        severity: 'Orta',
        date: '2026-07-08 14:30',
        status: 'Uyarı gönderildi',
      },
    ],
    staff: () => [
      {
    ...
    api: () => [
      ...
      },
      {
        id: 'api5',
        name: 'Eski REST API',
        endpoint: 'api.bachmain.com/v1',
        calls: '12K/ay',
        status: 'Kullanımdan Kaldırılıyor',
        version: '1.0',
      },
    ],
    settings: () => [
    ...
    formFields: [
      {
        ...
        type: 'select',
        required: true,
        options: customers.map((c: (typeof customers)[0]) => ({ label: c.company, value: c.id })),
      },
      {
        name: 'plan',
        label: 'Plan',
...
for (const def of moduleDefinitions) {
  ...
  moduleConfigs[def.id] = {
    ...
    metrics: [
        ...
        value: String(
          rows.filter(
            (r) =>
              ...
              r.status === 'Yayında' ||
              r.status === 'Gönderildi',
          ).length || Math.floor(rows.length * 0.7),
        ),
        change: '—',
        trend: 'up',
      },
      ...
      {
        label: 'Bu Ay',
        value: String(Math.floor(rows.length * 0.3) || 1),
        change: 'Yeni',
        trend: 'up',
      },
    ],
