...
const moduleDefinitions: Omit<ModuleConfig, 'metrics'>[] = [
  { id: 'customers', title: 'Müşteri Yönetimi', subtitle: 'Tüm müşteri hesaplarını yönetin', singularName: 'Müşteri', path: '/musteriler',
    columns: [
      { key: 'company', label: 'Firma', sortable: true }, { key: 'contact', label: 'Yetkili', sortable: true },
      { key: 'city', label: 'Şehir' }, { key: 'plan', label: 'Plan' }, { key: 'mrr', label: 'MRR' },
      { key: 'status', label: 'Durum' }, { key: 'licenseExpiry', label: 'Lisans Bitiş', sortable: true },
    ],
    formFields: [
  ...
  { id: 'subscriptions', title: 'Abonelik ve Lisanslar', subtitle: 'Aktif abonelikleri ve lisansları yönetin', singularName: 'Abonelik', path: '/abonelikler',
    columns: [{ key: 'customer', label: 'Müşteri', sortable: true }, { key: 'plan', label: 'Plan' }, { key: 'startDate', label: 'Başlangıç', sortable: true }, { key: 'expiry', label: 'Bitiş', sortable: true }, { key: 'users', label: 'Kullanıcı' }, { key: 'mrr', label: 'MRR' }, { key: 'status', label: 'Durum' }],
    formFields: [
  ...
  { id: 'payments', title: 'Tahsilatlar ve Ödemeler', subtitle: 'Tahsilat ve ödeme kayıtları', singularName: 'Tahsilat', path: '/tahsilatlar',
    columns: [{ key: 'customer', label: 'Müşteri', sortable: true }, { key: 'amount', label: 'Tutar', sortable: true }, { key: 'method', label: 'Yöntem' }, { key: 'date', label: 'Tarih', sortable: true }, { key: 'status', label: 'Durum' }],
    formFields: [
      { name: 'customer', label: 'Müşteri', type: 'select', required: true, options: customers.map((c: typeof customers[0]) => ({ label: c.company, value: c.id })) },
