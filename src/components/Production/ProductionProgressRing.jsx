...
export default function ProductionProgressRing({
  ...
  const center = size / 2
  const valueClass =
    size <= 40 ? 'text-[10px]' : size <= 52 ? 'text-[11px]' : 'text-[13px]'
  ...
  return (
    <div className={`inline-flex items-center gap-2 ${className}`.trim()}>
      ...
      >
        ...
        <span
          className={`absolute font-black tabular-nums text-[var(--bach-navy,#1E3A8A)] ${valueClass}`}
        >
          %{value}
        </span>
      </div>
      {label ? (
        <span className="text-[12px] font-semibold leading-tight text-[var(--muted,#64748B)]">
          {label}
        </span>
      ) : null}
    </div>
