    ...
    <div className="glass-inset rounded-[20px] p-4">
      <div className={`grid ${documentItemGridClass} ${documentItemFieldGapClass} items-start`}>
        ...
        <div className="flex min-h-0 flex-col gap-3 self-stretch">
          <div
            className={`grid ${documentItemFieldsGridClass} ${documentItemFieldGapClass} items-end`}
          >
            <DocumentField label="Ürün">
              <ProductSearchSelect
                item={item}
                onSelect={(product) => onSelectProduct(item.id, product)}
            ...
            <DocumentField label="Birim Fiyat">
              <NumericInput
                ...
                onChange={(value) => onUpdate(item.id, 'unitPrice', value)}
                suffix="₺"
                formatMode="price"
              />
            </DocumentField>
            <DocumentField label="KDV %">
              <select
                ...
                onChange={(event) => onUpdate(item.id, 'vatRate', Number(event.target.value))}
            ...
            <div className="relative">
              ...
              <div
                className={`flex h-[38px] items-center ${documentItemFieldGapClass}`}
                data-document-dropdown
              >
                <button
                  type="button"
                  ...
                  <div className="grid grid-cols-[minmax(0,1fr)_92px] items-end gap-3">
                    <DocumentField label="İndirim %">
                      <NumericInput
                        ...
                        onChange={(value) => onUpdate(item.id, 'discountRate', value)}
                      />
                    </DocumentField>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-[minmax(0,1fr)_92px] items-end gap-3">
                    <DocumentField label="ÖTV %">
                      <NumericInput
                        ...
                        onChange={(value) => onUpdate(item.id, 'exciseTaxRate', value)}
                      />
                    </DocumentField>
                    <button
                      type="button"
                      onClick={() =>
                        onDisableOption(item.id, 'showExciseTax', { exciseTaxRate: 0 })
                      }
                      className="inline-flex h-[38px] items-center justify-center gap-1.5 rounded-xl border border-red-500/20 bg-red-500/10 px-3 text-xs font-black text-red-300 transition-colors hover:bg-red-500/20"
                    >
                      <X className="h-3.5 w-3.5" /> Kaldır
                  ...
                  <div className="grid grid-cols-[minmax(0,1fr)_92px] items-end gap-3">
                    <DocumentField label="Konaklama Vergisi %">
                      <NumericInput
                        ...
                        onChange={(value) => onUpdate(item.id, 'accommodationTaxRate', value)}
                      />
                    </DocumentField>
                    <button
                      type="button"
                      onClick={() =>
                        onDisableOption(item.id, 'showAccommodationTax', {
          ...
          {item.showDescription && (
            <div
              className={`mt-auto grid grid-cols-[minmax(0,1fr)_92px] items-end ${documentItemFieldGapClass}`}
            >
              ...
              <button
                type="button"
                onClick={() =>
                  onDisableOption(item.id, 'showDescription', { extraDescription: '' })
                }
                className="inline-flex h-[38px] items-center justify-center gap-1.5 rounded-xl border border-red-500/20 bg-red-500/10 px-3 text-xs font-black text-red-300 transition-colors hover:bg-red-500/20"
              >
                <X className="h-3.5 w-3.5" /> Kaldır
