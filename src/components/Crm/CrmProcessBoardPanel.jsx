...
export default function CrmProcessBoardPanel({
  ...
  return (
    <AppPagePanel
      title="CRM Süreçleri"
      description="Görev ve randevu süreçlerini filtreleyin, aşamaları güncelleyin."
      action={(
      ...
      <div className="mt-4 space-y-2">
        {entries.length === 0 ? (
          <div className="rounded-xl border border-dashed border-dark-500/50 bg-dark-800/30 px-6 py-10 text-center">
            ...
            <p className="text-sm font-black text-white">Filtreye uygun süreç yok</p>
            <p className="mt-1 text-xs text-gray-500">Filtreleri gevşetin veya yeni görev / randevu ekleyin.</p>
          </div>
        ) : (
          entries.map((entry) => (
            <CrmProcessCard
