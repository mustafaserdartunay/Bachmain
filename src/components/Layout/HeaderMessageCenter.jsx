...
function SocialCountBadge({ count, size = 'md' }) {
  ...
  return (
    <span className={`social-dock-badge absolute -bottom-0.5 -right-0.5 flex items-center justify-center rounded-full bg-[#ff3b30] px-0.5 font-black leading-none text-white ring-white ${sizeClass}`}>
      {count > 99 ? '99+' : count}
    </span>
  )
}
...
function HeaderSocialIcon({ channel, count, onNavigate }) {
  ...
  return (
    ...
    >
      <span className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-lg shadow-sm ${SOCIAL_BRAND_BACKGROUNDS[channel.id]}`}>
        <Icon className="h-4 w-4" />
        <SocialCountBadge count={count} size="sm" />
      </span>
      <span className="w-full text-center text-[11px] font-bold leading-tight text-[var(--ink)]">{channel.label}</span>
    </Link>
  )
}
      ...
      {open && createPortal(
        ...
        >
          <div className="header-popover-head">
            <p className="text-sm font-extrabold text-[var(--ink)]">Mesaj Merkezi</p>
            <button
              type="button"
              onClick={() => {
                setOpen(false)
              ...
              className="btn-ghost shrink-0 !px-2.5 !py-1.5 text-[13px] font-bold"
            >
              Tümü
            </button>
          </div>
