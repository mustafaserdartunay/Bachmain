      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <AppPagePanel title="Kural ekle">
          <form onSubmit={addRule} className="space-y-3">
            <input className="form-input" value={draft.title} onChange={(e) => setDraft((c) => ({ ...c, title: e.target.value }))} />
            <select className="form-input" value={draft.cadence} onChange={(e) => setDraft((c) => ({ ...c, cadence: e.target.value }))}>
              {['Her gün', 'Her Pazartesi', 'Her Cuma', 'Her ay'].map((o) => <option key={o}>{o}</option>)}
            </select>
            <input type="time" className="form-input" value={draft.time} onChange={(e) => setDraft((c) => ({ ...c, time: e.target.value }))} />
            <select className="form-input" value={draft.action} onChange={(e) => setDraft((c) => ({ ...c, action: e.target.value }))}>
              {['Instagram paylaş', 'Blog yaz', 'LinkedIn paylaş', 'Newsletter hazırla'].map((o) => <option key={o}>{o}</option>)}
            </select>
        ...
        <AppPagePanel title="Aktif kurallar">
          <div className="space-y-2">
            {rules.length === 0 ? <p className="text-sm text-[var(--muted)]">Henüz kural yok.</p> : null}
            {rules.map((rule) => (
              <div key={rule.id} className="flex items-center justify-between rounded-xl border border-dark-500/40 px-3 py-2">
                <div>
                  <p className="text-sm font-semibold">{rule.title}</p>
                  <p className="text-[11px] text-[var(--muted)]">{rule.cadence} · {rule.time} · {rule.action}</p>
                </div>
