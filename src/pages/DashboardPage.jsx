  ...
  const depoInvoices = depoItems
    ...
    .map((item) => {
      ...
      return {
        ...
        total: net + vat,
        lines: [
          { product: item.product, quantity, unitPrice: item.unitPrice, vatRate: item.vatRate },
        ],
      }
    })

  const rows = [...treasuryInvoices, ...orderInvoices, ...shoppingInvoices, ...depoInvoices].filter(
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 line-clamp-1 text-[12px] font-semibold text-gray-500">
            {description}
          </p>
        </div>
        <span
          className={`flex h-8 w-8 items-center justify-center rounded-lg border border-dark-500/50 bg-dark-700/60 ${tone}`}
        >
      ...
      <p className="mt-1 text-[11px] font-black uppercase tracking-wide text-gray-500">
        {data.rows.length} detay kaydı görüntüle
      </p>
    </button>
  )
}
      ...
      <div className="max-h-[88vh] w-full max-w-5xl overflow-hidden rounded-3xl border border-dark-500/60 bg-dark-900 shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-dark-500/50 p-5">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-gray-500">
              Fatura ve kalem detayları KDV hariç, KDV ve KDV dahil olarak listelenir.
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border border-dark-500/50 p-2 text-gray-400 hover:bg-dark-700 hover:text-white"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="grid grid-cols-3 gap-3 border-b border-dark-500/50 p-5">
          <div className="rounded-2xl border border-dark-500/45 bg-dark-800/60 p-4">
            <p className="text-[12px] font-black uppercase tracking-wide text-gray-500">
              KDV Hariç
            </p>
          ...
          <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
            <p className="text-[12px] font-black uppercase tracking-wide text-emerald-300">
              KDV Dahil
            </p>
            ...
            <div className="space-y-3">
              {data.rows.map((row) => (
                <article
                  ...
                  className="rounded-2xl border border-dark-500/45 bg-dark-800/55 p-4"
                >
                    ...
                    <div className="mt-3 space-y-1 border-t border-dark-500/35 pt-3">
                      {row.lines.slice(0, 5).map((line, index) => (
                        <div
                          ...
                          className="flex justify-between gap-3 text-[13px] font-semibold text-gray-500"
                        >
                          <span className="truncate">
                            {line.product || line.name || line.description || 'Kalem'} ·{' '}
                            {Number(line.quantity) || 0} adet · KDV %
                            {Number(line.vatRate ?? line.vat) || 0}
                          </span>
...
function ProcessMetricCard({
  ...
  return (
    <article className={`rounded-2xl border ${config.border} bg-dark-800/75 p-4 shadow-card`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-[12px] font-black uppercase tracking-[0.18em] text-gray-400">
            {config.badge}
          </p>
          <h2 className="mt-1 truncate text-sm font-black text-gray-100">{title}</h2>
        </div>
        <span
          className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ${config.iconBox}`}
        >
          <Icon className="h-4 w-4" />
        </span>
      </div>
...
function DashboardSection({ title, subtitle, icon: Icon, children, className = '' }) {
  return (
    <section
      className={`overflow-hidden rounded-3xl border border-dark-500/50 bg-dark-800/75 shadow-card ${className}`}
    >
...
function CrmOverviewCard() {
  ...
  return (
    <DashboardSection
      title="CRM Bilgileri"
      subtitle="Görev, randevu ve takip özeti"
      icon={UsersRound}
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {tiles.map((tile) => {
          ...
          return (
            <article
              key={tile.label}
              className="rounded-2xl border border-dark-500/45 bg-dark-700/40 p-4"
            >
              <div className="flex items-center justify-between gap-3">
                <p className="text-[12px] font-black uppercase tracking-[0.18em] text-gray-400">
                  {tile.label}
                </p>
    ...
    <div className="flex flex-wrap items-center justify-end gap-2">
      {tiles.map((tile) => {
        ...
        return (
          <span
            key={tile.label}
            className="inline-flex h-9 items-center gap-2 rounded-xl border border-dark-500/45 bg-dark-700/40 px-3 text-xs font-black text-gray-400"
          >
...
function DashboardNotesPanel({ entries, onSubmit, onDelete, onReorder, onToggleComplete }) {
  ...
  const [draft, setDraft] = useState(() =>
    entries.length === 0 ? createEmptyDashboardNote(loadRawNoteProcessTemplates()) : null,
  )
  ...
  function startEdit(entry) {
    const template =
      noteTemplates[entry.record.noteTemplateId] || getFirstNoteTemplate(noteTemplates)
    const stage =
      template?.stages?.find(
        (item) => item.id === entry.record.statusId || item.label === entry.record.status,
      ) || template?.stages?.[0]
        ...
        <div className="space-y-2">
          {isEditing && (
            <form
              ...
              className="rounded-xl border border-dark-500/45 bg-dark-700/35 p-3"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <p className="mb-1 text-[12px] font-black text-blue-300/80">
                    {formatNoteTimestamp(draft)}
                  </p>
                  <textarea
                    autoFocus
                    value={draft.content}
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    type="submit"
                    className="inline-flex rounded-lg p-1 text-blue-300 transition-colors hover:bg-blue-500/10"
          ...
          {entries.map((entry, index) => {
            ...
            return (
              <article
                key={entry.id}
                className={`rounded-xl border p-3 ${isCompleted ? 'border-emerald-500/25 bg-emerald-500/5' : 'border-dark-500/45 bg-dark-700/35'}`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p
                      className={`line-clamp-2 text-sm font-black text-white decoration-2 decoration-emerald-300/80 ${isCompleted ? 'text-gray-400 line-through' : ''}`}
                    >
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    ...
                    <button
                      ...
                      disabled={index === 0}
                      className="rounded-lg p-1 text-gray-500 hover:bg-dark-600 hover:text-white disabled:opacity-30"
                    >
                    ...
                    <button
                      ...
                      className="rounded-lg p-1 text-blue-300 hover:bg-blue-500/10"
                    >
                        ...
                        <div className="absolute right-0 top-7 z-20 w-36 rounded-xl border border-red-500/25 bg-dark-800 p-2 shadow-card">
                          ...
                          <div className="mt-2 flex gap-1">
                            <button
                              ...
                              className="flex-1 rounded-lg border border-dark-500/50 px-2 py-1 text-[12px] font-black text-gray-400"
                            >
                              Hayır
                            </button>
                            <button
