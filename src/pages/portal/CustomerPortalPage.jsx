        ...
        <div className="flex flex-wrap items-center justify-between gap-4 px-4 sm:px-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="rounded-xl border border-blue-500/25 bg-blue-500/10 px-3 py-2 text-right">
              <p className="text-[10px] font-black uppercase tracking-wider text-blue-300">Güncel Bakiye</p>
        ...
        <aside className="card h-fit space-y-1 p-2">
          {TABS.map(({ id, label, icon: Icon }) => (
            ...
            >
              ...
              {label}
              {id === 'cart' && cartCount > 0 && (
                <span className="ml-auto rounded-md bg-blue-500/20 px-1.5 py-0.5 text-[10px] text-blue-200">{cartCount}</span>
              )}
            </button>
          ))}
        </aside>
        ...
        <main className="space-y-4">
          ...
          {tab === 'cari' && (
            <section className="card overflow-hidden p-0">
              <div className="grid grid-cols-[130px_130px_minmax(0,1fr)_120px_110px] border-b border-dark-500/45 px-5 py-3 text-[11px] font-black uppercase tracking-wider text-gray-500">
          ...
          {tab === 'products' && (
            <section className="space-y-4">
              <div className="card">
                <p className="mb-3 text-[10px] font-black uppercase tracking-wider text-gray-500">Kategori</p>
              ...
              <section className="card overflow-hidden p-0">
                <div className="grid grid-cols-[72px_minmax(0,1.4fr)_120px_100px_110px_120px] border-b border-dark-500/45 px-4 py-3 text-[10px] font-black uppercase tracking-wider text-gray-500">
                  <span>Görsel</span>
                  <span>Ürün</span>
                  <span>Kategori</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="truncate text-[11px] font-semibold text-gray-500">{product.stockCode}</p>
                          {inCart && (
                            <p className="mt-0.5 text-[10px] font-bold text-blue-300">Sepette: {inCart.quantity} adet</p>
                          )}
                        </div>
                        ...
                        <div className="text-right">
                          ...
                          <span className={`inline-block rounded-md px-1.5 py-0.5 text-[9px] font-black uppercase ${stockInfo.badge}`}>
                            {stockInfo.label}
                          </span>
                        </div>
                        <div className="text-right">
                          ...
                          {isDealer && price !== listPrice && (
                            <p className="text-[10px] text-gray-500 line-through">{formatTL(listPrice)}</p>
                          )}
                        </div>
                        <div className="flex justify-end">
                          <button
                            ...
                            className="rounded-xl border border-blue-500/30 bg-blue-500/10 px-2.5 py-2 text-[10px] font-black uppercase text-blue-200 transition-colors hover:bg-blue-500/20"
                          >
                            Sepete Ekle
                          </button>
                        </div>
                  ...
                  <div className="card space-y-3">
                    ...
                    {paymentMethod === 'havale' && (
                      <div className="space-y-2 rounded-2xl border border-dark-500/40 bg-dark-700/30 p-4">
                        <p className="text-[10px] font-black uppercase text-gray-500">Havale Bilgileri</p>
                        {visibleBankAccounts.map((account) => (
                          <div key={account.id} className="rounded-xl border border-dark-500/35 px-3 py-2">
                            ...
                            <p className="text-[11px] text-gray-500">{account.iban}</p>
                          </div>
                        ))}
                        <p className="text-[11px] text-gray-500">Açıklama kısmına sipariş numaranızı yazınız.</p>
                      </div>
                    )}
              ...
              {[...orders.map((o) => ({ ...o, kind: 'Sipariş' })), ...quotes.map((q) => ({ ...q, kind: 'Teklif' }))].map((item) => (
                <article key={item.id} className="card">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-lg bg-blue-500/15 px-2 py-1 text-[10px] font-black uppercase text-blue-300">{item.status}</span>
                  </div>
              ...
              {tickets.map((ticket) => (
                <article key={ticket.id} className="card space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-md bg-emerald-500/15 px-2 py-0.5 text-[10px] font-black text-emerald-300">{ticket.status}</span>
                  </div>
                  ...
                  {(ticket.replies || []).map((reply) => (
                    <div key={reply.id} className="rounded-xl border border-blue-500/20 bg-blue-500/10 px-3 py-2">
                      <p className="text-[10px] font-black uppercase text-blue-300">{reply.author}</p>
          ...
          {tab === 'profile' && (
            <section className="grid gap-4 lg:grid-cols-2">
              <div className="card space-y-3">
                ...
                {isDealer && <span className="inline-flex rounded-lg bg-emerald-500/15 px-2 py-1 text-[10px] font-black uppercase text-emerald-300">Bayi Hesabı</span>}
              </div>
              <div className="card space-y-3">
                ...
                <div className="space-y-2 pt-2">
                  {visibleBankAccounts.map((account) => (
                    <div key={account.id} className="rounded-xl border border-dark-500/40 bg-dark-700/30 px-3 py-2">
                      ...
                      <p className="mt-1 text-[11px] font-semibold text-gray-500">{account.iban}</p>
                    </div>
                  ))}
                </div>
              </div>
