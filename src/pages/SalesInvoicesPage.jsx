...
export default function SalesInvoicesPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener(SALES_INVOICES_EVENT, refresh)
  ...
  useEffect(() => {
    ...
    navigate('/musteriler/faturalar/yeni')
  ...
  function handleQuickDraft() {
    ...
    const invoice = createSalesInvoice({
      ...
      totalAmount: 0,
      status: 'draft',
      invoiceKind: customer?.eInvoiceType === 'e-arsiv' ? 'e-arsiv' : 'e-fatura',
      syncTreasury: false,
    })
  ...
  function handleCustomerSelect(customer) {
    ...
    navigate(`/musteriler/faturalar/yeni?musteri=${customer.id}`)
  }
      ...
      <AppPageHeader
        ...
        actions={
          <SplitCreateButton
            ...
            onPrimaryClick={() => navigate('/musteriler/faturalar/yeni')}
            menuAriaLabel="Fatura seçenekleri"
            menuItems={[
              {
                id: 'create',
                label: 'e-Fatura / e-Arşiv Kes',
                icon: Receipt,
                iconClassName: 'text-blue-300',
                onClick: () => navigate('/musteriler/faturalar/yeni'),
              },
              {
                id: 'customer',
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="flex items-start gap-3">
                    <Link
                      to={`/musteriler/faturalar/${invoice.id}`}
                      className="relative mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-dark-500/45 bg-dark-700/80 text-gray-400 hover:border-blue-500/40 hover:text-blue-300"
                    >
                      <FileText className="h-5 w-5" />
                      {(invoice.invoiceKind === 'e-fatura' ||
                        invoice.invoiceKind === 'e-arsiv') && (
                        <span
                          className={`absolute -bottom-1 -right-1 rounded px-1 text-[10px] font-black text-white ${
                            invoice.invoiceKind === 'e-arsiv' ? 'bg-violet-500' : 'bg-blue-500'
                          }`}
                        >
                          e
                        </span>
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <Link
                          to={`/musteriler/faturalar/${invoice.id}`}
                          className="truncate text-sm font-black text-white hover:text-blue-300"
                        >
                          {invoice.title}
                        </Link>
                        <span
                ...
                <div>
                  ...
                  {invoice.gibStatus && invoice.gibStatus !== 'idle' ? (
                    <p className="mt-1 text-[11px] font-semibold text-blue-300/90">
                      GİB: {GIB_STATUS_LABELS[invoice.gibStatus] || invoice.gibStatus}
                      {invoice.emailStatus && invoice.emailStatus !== 'idle'
                        ? ` · Mail: ${EMAIL_STATUS_LABELS[invoice.emailStatus] || invoice.emailStatus}`
                        : ''}
                    </p>
                  ) : null}
                </div>
