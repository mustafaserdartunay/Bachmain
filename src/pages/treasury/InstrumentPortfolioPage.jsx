...
export default function InstrumentPortfolioPage({
  accountType = 'Çek Kasası',
  title = 'Çekler',
  noun = 'Çek',
  tone = 'purple',
}) {
  ...
  const rows = useMemo(() => {
    ...
    return collectAllChequeRows(accountType).filter((row) => {
  ...
  return (
    <AppPageShell>
      ...
      <SummaryMetrics
        columns={3}
        items={[
          { title: `Portföydeki ${noun}`, value: rows.length, icon: ScrollText, tone, valueTone: tone },
          { title: 'Toplam Tutar', value: formatTL(portfolioTotal), icon: Landmark, tone: 'blue', valueTone: 'blue' },
          {
            title: accountType,
            value: accounts.filter((item) => item.type === accountType).length,
            icon: ScrollText,
            tone: 'emerald',
            valueTone: 'emerald',
          },
      ...
      <AppPagePanel title={`${noun} Portföyü`}>
        <SearchInput
          wrapperClassName="mb-4 w-full max-w-md"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder={`${noun} no, banka veya cari ara...`}
        />

        <ListHeaderRow
          gridTemplate={LIST_GRID}
          columns={['Banka', `${noun} No`, 'Vade', 'Cari', { label: 'Tutar', align: 'right' }]}
        />

        <div className="mt-2 space-y-2">
          {rows.length === 0 ? (
            <p className="py-10 text-center text-sm text-gray-500">{noun} kaydı bulunamadı.</p>
          ) : rows.map((row) => (
            <Link
              key={row.id}
            ...
            >
              ...
              <p className={`text-right text-sm font-black ${tone === 'amber' ? 'text-amber-300' : 'text-purple-300'}`}>
                {formatTL(row.amount)}
              </p>
            </Link>
          ))}
