...
export function readIncomingEInvoices() {
  try {
    ...
    const needsUpgrade = saved.some(
      (item) => !Array.isArray(item?.lines) || item.lines.length === 0,
    )
    if (needsUpgrade) {
      const byNo = new Map(SEED.map((item) => [item.invoiceNo, item]))
      const merged = saved.map((item) => {
        const seed = byNo.get(item.invoiceNo)
        return normalizeInvoice(
          seed ? { ...seed, ...item, lines: item.lines?.length ? item.lines : seed.lines } : item,
        )
      })
      localStorage.setItem(INCOMING_E_INVOICES_KEY, JSON.stringify(merged))
      return merged
    }
    return saved.map(normalizeInvoice).filter(Boolean)
  } catch {
    return SEED.map(normalizeInvoice)
  }
}
