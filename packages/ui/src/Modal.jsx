...
export function Modal({ open, onClose, title, children, footer, size = 'md', className = '' }) {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`relative z-10 w-full ${maxW} overflow-hidden rounded-ds-dialog border border-ds-border bg-ds-surface shadow-ds-layer-3 ${className}`}
      >
        ...
        {footer ? (
          <div className="flex flex-wrap justify-end gap-2 border-t border-ds-border px-5 py-4">
            {footer}
          </div>
        ) : null}
      </div>
